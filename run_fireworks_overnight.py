#!/usr/bin/env python3
"""Overnight: deepseek-r1-distill-32b, deepseek-v3.2, glm-4.7, mixtral-8x22b. Zero babysitting."""
...
MODELS = [
    ...
    ("fireworks",  "accounts/fireworks/models/mixtral-8x22b-instruct", "mixtral-8x22b"),
]
...
def import_csv(path, canonical_name, dataset_label, conn):
    ...
    if not rows:
        ...
        return 0
    provider = PROVIDER_MAP.get(canonical_name, "fireworks")
...
for api, model_id, canonical_name in MODELS:
    ...
    for data_file, dataset_label, expected_probes in DATASETS:
        for temp in TEMPS:
            ...
            out_file = f"fw_{label}_{dataset_label}_t{temp.replace('.','')}.csv"
...
print("\nAll Fireworks/OpenRouter models done.", flush=True)
conn.close()
